...
class wl_wishList extends ObjectModel
{
    /** @var int Wishlist ID */
    public $id;
    ...
    public static function checkAddedToWishlist($id_product, $id_product_attribute)
    {
        $result = Db::getInstance()->getRow(
            '
            SELECT wp.`quantity`,w.`id_wishlist`
                FROM `' . _DB_PREFIX_ . 'wishlist_product` wp
            JOIN `' . _DB_PREFIX_ . 'wishlist` w ON (w.`id_wishlist` = wp.`id_wishlist`)
    ...
    public static function removeProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute)
    {
        if ($id_wishlist) {
            ...
            return Db::getInstance()->delete(
                'wishlist_product',
                'id_wishlist = ' . (int) $result['id_wishlist'] . ' AND id_product = ' . (int) $id_product . ' AND id_product_attribute = ' . (int) $id_product_attribute
            );
        } else {
            ...
            if ($result) {
                foreach ($result as $item) {
                    Db::getInstance()->delete(
                        'wishlist_product_cart',
                        'id_wishlist_product = ' . (int) $item['id_wishlist_product']
                    );
                    Db::getInstance()->delete(
                        'wishlist_product',
                        'id_wishlist = ' . (int) $item['id_wishlist'] . ' AND id_product = ' . (int) $id_product . ' AND id_product_attribute = ' . (int) $id_product_attribute
                    );
                }
            }
            return true;
    ...
    public static function getProductByIdCustomer($id_wishlist, $id_customer, $id_lang, $id_product = null, $quantity = false)
    {
        ...
        for ($i = 0; $i < sizeof($products); ++$i) {
            if (
                isset($products[$i]['id_product_attribute']) &&
    ...
    public function getProductsOrCount(
        ...
        $querySearch->where('wp.id_wishlist = ' . (int) $this->id);
    ...
    {
        $sql = 'SELECT wp.id_product,wp.id_product_attribute,pl.name FROM `' . _DB_PREFIX_ . 'wishlist_product` wp
        ...
        if ($products) {
            ...
            return Context::getContext()->smarty->fetch(_PS_MODULE_DIR_ . 'wishlist_pres/views/templates/hook/images.tpl');
        }
    }
}
